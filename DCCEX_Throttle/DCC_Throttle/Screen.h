...
/*
    ...
    scr.cls         Clear screen to background or a colour & set header/footer
    scr.status      Set footer text
...
public:
    ...
    //  Swap foreground/background for inverse characters
    //
    void inverseIf(bool test) {
        if(test) {
            inverse();
        } else {
            normal();
    ...
    void inverse() {
        if(isInverse) return;                 // --> already inverse
        isInverse = true;
        c64 oldFg = currentFg;
        currentFg = currentBg;
        currentBg = oldFg;
    ...
    void normal() {
        if(!isInverse) return;                // --> already normal
        isInverse = false;
        c64 oldFg = currentFg;
        currentFg = currentBg;
        currentBg = oldFg;
    ...
    //  Text string
    //  Optional colours
    //  at(x,y,"text")              plain text
    //  at(x,y,"\xe7\xf1")          hex chars
    //
    ...
    // e.g.
    //      scr.at(0, 0, "Loco: %u  Speed: %d", locoAddress, speedStep);
    //      scr.at(0, 1, "Power: %s", trackPowerIsOn ? "ONLINE" : "OFF");
    //      scr.at(10, 5, "ERR: 0x%02X", errorCode); 
    //   
    template<typename T, typename... Args>
    void at(int x, int y, const char* format, T firstArg, Args... rest) {
        char buffer[COLS + 1];
        snprintf(buffer, sizeof(buffer), format, firstArg, rest...);        // Pass firstArg and rest... into snprintf safely
        at(x, y, buffer);
    }
