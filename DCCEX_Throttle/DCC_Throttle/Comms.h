  ...
  bool connectWiFi(const char* ssid, const char* password) {
    ...
    for (int attempts = 0; attempts < 20; attempts++) {
      if (WiFi.status() == WL_CONNECTED) {
        Terminal.log("\x06" " Connected");
        return true;                                    // --> Success
      }
      delay(300);
    }

    Terminal.log("\x06" " Failed");
...
public:
  ...
  //  Send data in printf format with embedded vars
  //
  void dccPrintf(const char* format, ...) {
    char buffer[128];
    va_list args;
    ...
    dccClient.print(buffer);                          // Send it

    if (len > 0 && buffer[len - 1] == '\n') {
        buffer[len - 1] = '\0';
    }
    Terminal.log(buffer, true);                       // Log outbound on terminal
  }

  //  Propogate the connection state
  //
  bool dccConnected() {
    return dccClient.connected();
  }


  //  Propogate retreival of content
  //
  bool dccAvailable() {
    return dccClient.available();
  }

  char dccRead() {
    return dccClient.read();
  }
